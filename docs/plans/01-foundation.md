...
**Executor Notes:**
- Layer 1 `openai` pattern uses negative lookbehind `(?<![a-zA-Z0-9])sk-[A-Za-z0-9\-_]{20,}` so `sk-` is not preceded by an alphanumeric character, preventing false-positive redaction on title-case, digit-containing, or lowercase hyphenated prose words and dict keys (e.g. `Task-Management-System-Config`, `risk-assessment-protocol-2024-version`, `disk-space-warning-threshold-v2`).
- `test_redaction_never_raises_and_never_leaks` tests secret redaction within arbitrary text payloads generated by Hypothesis.
- Short-circuit threshold is `len(text) < 8`; `_safe_text_cache` caches only non-secret strings (where `res == text`) to satisfy the performance budget without caching secrets in memory.
...
- `uv run pytest python/chowki/tests/benchmarks --benchmark-only -q` →
  `test_full_snapshot_1mib_within_total_budget` median **< 3.0 ms** (2.0 × 1.5) and the
