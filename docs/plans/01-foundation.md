...
- attempt 3 (escalation): Drop nonce expiry GC entirely so both adapters treat consume_nonce as lifetime single-use; add contract test A/B/A -> (True, True, False); centralise blob ref derivation in `chowki.state.blobs.make_blob_ref`; add the `status` column to the `idempotency` table; normalize audit records via msgpack in MemoryStorage -> PASS
