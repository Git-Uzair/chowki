...
| Args-hash | Canonical content hash of sanitized `{name, args, kwargs}`; NFC keys, total-ordered sets, `<TypeName>` fallback (collapse caveat), cycle markers, non-finite-float markers. | 07 §4 | ✅ | ⬜ |
...
| User guide + flagship example + packaging | `docs/user-guide/`, the showcase agent (budget pause, approval gate, kill → `rerun`), CHANGELOG, PyPI release. | 02-release T6–T8 | ✅ (User guide, Showcase agent) | — |
