...
| Workflow runner (`workflow`) | Opens/loads the run, injects `run_id`/`tenant_id` kwargs (reserved names), binds an engine, drives sync/async bodies, persists the first call's arguments (redacted) on the run record and replays them on resume/rerun, persists terminal status + usage, snapshots final state. | 07 §9 | ✅ | ⬜ |
