...
> **Amendment (2026-08-12, normative): how row 3 and cold resume are enforced.**
>
> **Row 3 (canonical hashing) is gated relatively, not absolutely.** Hashing 1 MiB *is* OpenSSL's SHA-256 core: chowki's own contribution — the hex encode and the `sha256:` prefix — measures 0.9 µs, 0.2% of the total. The absolute median therefore reports which CPU the build landed on, not whether the library regressed: 0.482 ms (2.18 GB/s) on the reference dev box against 0.751 ms (1.40 GB/s) on a shared GitHub runner, a 1.6× spread no chowki change can move. `canonical_hash_1mb_ms` = 0.35 ms is retained as the reference figure; enforcement is `HASH_OVERHEAD_MAX` = 1.15, a ceiling on `hash_bytes` measured against the platform digest sampled alternately on the same CPU, so runner throughput cancels out. Measured ratio is 1.000 ± 0.006 across trials and 2.008 for a regression that walks the buffer twice. `canonical_hash_1mb_ceiling_ms` = 1.0 ms remains as a loose absolute backstop.
>
> **Cold resume has its own gate.** `warm_resume_base_plus_10_deltas_ms` = 2.5 ms covers `DeltaChain.materialize()` over trees already in memory (0.064 ms measured). Reconstructing a run from stored envelopes, as `chowki resume` does, is a different operation and is gated by `cold_load_base_plus_10_deltas_ms` = 5.0 ms (7.5 ms after tolerance), set from measurement: 4.33 ms on the reference dev box, 4.95 ms on a shared runner. It runs once per resume, not per step. The composition — msgpack decode of the 1 MiB base 0.85 ms, chain replay 0.067 ms, the `inline_blobs` walk 2.08 ms, the defensive `_copy_containers` walk 1.31 ms — leaves the two tree walks at 77% of the total and is the first place to look for cold-path headroom.
